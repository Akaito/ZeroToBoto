...
This always returns a boolean value `True` or `False`.
...
`'hello'` is `True`.  `'False'` (note the string-ness) is even `True`.
...
*Unlike* many other languages, Python sort of "leaks" that <tt>&lt;variable-name&gt;</tt> outside of the `for .. in` loop, as seen in the last line of the above example.
The chosen variable name continues to *refer to* to the last value in the list (it's not a second copy of it).
...
Similarly, types drive the behavior of `for .. in` loops.
...
Literally meaning you can't change a `str`, and you can change `list`.
Above, what *looks* like successfully adding an exclamation point to <tt>message</tt> is actually making a new `str`, and making the variable <tt>message</tt> refer to the new one instead of the old one; the old one is discarded.
